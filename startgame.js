...
async function startGameplay() {
  ...
  await creayDialog(
    `${name}: После серии взрывов я сижу тут месяц. Надо бы выйти на поверхность за припасами`
  );
  await creayDialog(
    `${name}: Мне уже плевать если я помру там, мой ЁЖ скорее всего погиб после взрыва`
  );
...
async function exetBunker() {
  ...
  video.loop = true;
  await creayDialog(
    'О я вижу в далеке табличку. Как же поступить, проведать свой прошлый дом или же подойти к табличке?'
  );
  await choiceDialog2(
    'куда пойти',
    'Проведать дом',
...
async function stopHouse() {
  ...
  video.loop = true;
  await creayDialog(
    'Мой дом уничтожен. Надо попробовать сыграть на гитаре, вдруг ежик вернется'
  );
  goHedgehog();
}
...
async function goHedgehog() {
  ...
  await creayDialog(
    'Цель: спасти ежа. Теперь ты идешь один. За каждый бой уровень радиации будет расти — покупай антирадин (уколы), чтобы снижать её. Для восстановления здоровья используй аптечки. Повышай уровень, сражаясь с противниками, или качай характеристики в спортзале.'
  );
