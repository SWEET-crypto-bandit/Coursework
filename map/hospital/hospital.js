function miniGameNeedles() {
  ...
  function gameLoop() {
    ...
    needles.forEach(needle => {
      if (!needle.collected) {
        ...
        if (dist < player.radius + needle.radius) {
          ...
          score++;
          document.querySelector('.mini-game-score').textContent = `Иголок ${score}/${GOAL_SCORE}`;
          playMisuc4(); 
    ...
    if (mellstroy.x + mellstroy.radius > canvas.width || mellstroy.x - mellstroy.radius < 0) {
      mellstroy.speedX *= -1;
    }
    if (mellstroy.y + mellstroy.radius > canvas.height || mellstroy.y - mellstroy.radius < 0) {
      mellstroy.speedY *= -1;
    }
    ...
    ctx.fillStyle = '#000';
    ctx.beginPath();
    ctx.arc(mellstroy.x - 7 + (player.x > mellstroy.x ? 2 : -2), mellstroy.y - 5, 2, 0, Math.PI * 2);
    ctx.arc(mellstroy.x + 7 + (player.x > mellstroy.x ? 2 : -2), mellstroy.y - 5, 2, 0, Math.PI * 2);
    ctx.fill();
    ...
    const distToEnemy = Math.hypot(player.x - mellstroy.x, player.y - mellstroy.y);
  ...
  function endGame(reason) {
    ...
    ctx.fillStyle = '#fff';
    ctx.fillText(`Собрано иголок из своего тела ${score}/${GOAL_SCORE}`, canvas.width / 2, canvas.height / 2 + 20);
